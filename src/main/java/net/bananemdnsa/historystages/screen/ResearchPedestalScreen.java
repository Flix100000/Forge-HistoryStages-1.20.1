...
import net.bananemdnsa.historystages.network.PedestalControlPacket;
...
public class ResearchPedestalScreen extends AbstractContainerScreen<ResearchPedestalMenu> {
    private static final ResourceLocation[] TIER_TEXTURES = {
            HistoryStages.location("textures/gui/pedestal/gui_t1.png"),
            HistoryStages.location("textures/gui/pedestal/gui_t2.png"),
            HistoryStages.location("textures/gui/pedestal/gui_t3.png"),
            HistoryStages.location("textures/gui/pedestal/gui_t4.png"),
    };
    private static final ResourceLocation TEXTURE_BAR = HistoryStages.location("textures/gui/pedestal/bar.png");
    private static final ResourceLocation TEXTURE_DEP = HistoryStages.location("textures/gui/pedestal/dep_gui.png");
    private static final ResourceLocation TEXTURE_BUTTONS = HistoryStages.location("textures/gui/pedestal/buttons.png");
    ...
    private boolean canPressStart() {
        ...
        if (stack.isEmpty() || !stack.hasTag() || !stack.getTag().contains("StageResearch")) return false;
        ...
        if (this.menu.isTierMismatch()) return false; // pedestal tier does not satisfy the stage
        return this.menu.areDependenciesMet();
    }
    ...
    protected void init() {
        ...
        this.addRenderableWidget(new PedestalIconButton(
                ...
                () -> PacketHandler.sendToServer(
                        new PedestalControlPacket(this.menu.getBlockPos(), !this.menu.isRunning()))));
    }
     ...
     */
    private static boolean canAfford(DependencyResult.EntryResult entry) {
    ...
    private void renderDependencyPanel(GuiGraphics guiGraphics, int x, int y, int mouseX, int mouseY) {
        ...
        if (dDelay > 0) {
            ...
            if (!depositStack.isEmpty()) {
                ResourceLocation dRl = ForgeRegistries.ITEMS.getKey(depositStack.getItem());
