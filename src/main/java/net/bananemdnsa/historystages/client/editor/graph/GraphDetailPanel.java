...
public final class GraphDetailPanel {
    ...
    private sealed interface ContentRow permits StringRow, WrappedRow {
        int height();
    }
    ...
    private static void drawRow(GuiGraphics g, Font font, ContentRow row, int x, int y) {
        switch (row) {
            ...
            case WrappedRow r -> g.drawString(font, r.text(), x, y, r.color(), false);
        }
    }
     ...
     * because Task 13 is scoped to creating only {@code GraphSidebar} and {@code GraphDetailPanel};
